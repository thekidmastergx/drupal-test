 ...
 * Delete agenda
 */
function agenda_admin_delete($form_state, $delta) {
  $form = array();
...
function agenda_admin_configure($form_state, $delta) {
  ...
  $form['agenda_timezone'] = array(
    '#type'           => 'textfield',
    '#title'          => t('Timezone'),
    '#default_value'  => agenda_variable_get($delta, 'timezone', variable_get('date_default_timezone_name', date_default_timezone_get())),
...
function agenda_debug($bid) {
  ...
  $debug_title = array('#markup' => '<h2>' . t('Debugging %calendar block', array('%calendar' => $block->title)) . ' - '.l('Edit','admin/settings/agenda/'.$bid.'/configure',array('query' => drupal_get_destination())).'</h2>');
  ...
  if ($number_of_events === 0) {
    ...
    return array(
      'title'       => array('#markup' => '<h2>' . t('Debugging %calendar block', array('%calendar' => $block->title)) . ' - '.l('Edit','admin/settings/agenda/'.$bid.'/configure').'</h2>'),
